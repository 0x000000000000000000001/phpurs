...
await build({
  ...
  plugins: [{ name: 'disable-copy-cleanup', setup(b) {
    b.onLoad({ filter: /Phpurs\.CopyCleanup\/index\.js$/ }, () => ({ contents: "export const optimize = _ => file => file;", loader: 'js', resolveDir: path.join(backend, 'output/Phpurs.CopyCleanup') }));
  }}],
});
