...
for name in ['baseline','integrated']:
    ...
    assert s.count('  $__res = 0;')==(1 if name == 'baseline' else 0)
    s = s.replace('  $__res = 0;', "++$GLOBALS['fusion_counts']['seeds'];\n  $__res = 0;")
    if name == 'integrated':
        for before,counter in [('  $__tco_1 = ($value_1 + 1);', 'additions')]:
...
assert rows['integrated']=={'builders':0,'nodes':0,'node_calls':0,'seeds':0,'additions':1000000}
