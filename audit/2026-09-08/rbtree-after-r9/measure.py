...
names = sys.argv[2:] or list(json.loads((root / 'variants.json').read_text()))
...
checks = {}
for name in names:
    result = subprocess.run(flags + [str(root/'validate.php'), str(root/'raw'/name)], text=True, capture_output=True)
...
(root / ('validation-' + mode + '.json')).write_text(json.dumps(checks, indent=2) + '\n')
print('Canonical trees and retained roots agree.', flush=True)
rows = []
for round_number, order in enumerate([names, list(reversed(names))], 1):
    for name in order:
        command = flags + [str(root/'runner.php'), str(root/'raw'/name), mode]
        result = subprocess.run(command, text=True, capture_output=True)
        (root / f'{mode}-{round_number}-{name}.log').write_text(result.stdout + result.stderr)
        assert result.returncode == 0 and not result.stderr, (name, result.stdout, result.stderr)
        ...
        rows.append(row)
        (root / (mode + '-results.json')).write_text(json.dumps(rows, indent=2) + '\n')
