...
checks = {}
for name in names:
    result = subprocess.run(flags + [str(root/'validate-rbtree-public.php'), str(root/'raw'/name/'output')], text=True, capture_output=True)
...
for round_number, order in enumerate([names, list(reversed(names))], 1):
    for name in order:
        command = flags + [str(root/'rbtree-runner.php'), str(root/'raw'/name/'output'), mode]
