...
5. **Prochaine priorité : R11, fusion construction/évaluation des thunks.** Le [prototype isolé](audit/2026-09-09/lazy-fusion/report.md) donne Lazy 0,347–0,372 ms contre 46,959–47,062 ms dans la suite complète ; total 196,6–197,7 ms contre 245,3–245,6 ms. Les 14 sorties et les observations couvertes passent ; les callbacks inspectant la pile doivent être refusés. Porter la preuve et la transformation sur les AST avant de compter ce gain comme intégré.
6. B1–B4 : accélérer la boucle de développement, avec mesures de compilation séparées.
...
Le README actuel conserve le relevé utilisateur **316,45 ms**, avec RBTree **247,881 ms** ; les témoins actuels sont proches. Le natif optimisé historique donne RBTree 123,096 ms et total 200,45 ms ; le natif de style fonctionnel ne réalise pas exactement la même charge RBTree. Les expériences sur copies et les mesures du générateur intégré sont distinguées dans les bilans.
...
- [ ] **Suite R10 : `E = null`, après R11.** Prouver séparément le domaine fermé et l'absence de collision avec une autre représentation dans les régions R10, préserver les frontières et anciennes racines, puis remesurer avant extension.

## R11 — P1 — Fusion des chaînes de thunks immédiatement forcées

**Prototype isolé mesuré, générateur inchangé.** Lazy passe de 46,959–47,062 à 0,347–0,372 ms dans les 14 benchmarks ; le total de 245,328–245,621 à 196,584–197,720 ms, soit 19,50–19,87 % de temps total en moins. Le README actuel du checkout principal indique 237,07 ms / 44,911 ms ; le natif optimisé historique donne 200,45 ms / 0,368 ms. Les comparaisons utilisent les témoins contemporains. Voir le [bilan](audit/2026-09-09/lazy-fusion/report.md).
...
- [ ] **Prochaine petite étape : passe AST générique bornée.** Reconnaître un constructeur récursif de chaîne immédiatement consommée, avec corps uniforme prouvé, fonction initiale connue pure et profondeur admissible. Conserver le lien entre preuve typée et site après PBO ; respecter la signature aplatie et l'arité réelle du constructeur. Refuser callbacks inconnus/FFI, captures variables, fonctions conservées et cas non prouvés ; garder leurs chemins publics.
- [ ] Rejouer les contre-exemples et R8/R9/R10, régénérer avec `bin/php/run -c`, puis comparer à passe désactivée. Présenter séparément les gains du prototype et ceux de l'intégration.
