  ...
  return dictTraversable1 => {
    ...
    return {
      traverse: dictApplicative => {
        ...
        const traverse5 = dictTraversable1.traverse(dictApplicative);
        return f => {
          const $3 = Control$dSemigroupoid.composeImpl($2.map(Control$dSemigroupoid.composeImpl(Data$dFunctor$dCoproduct.Coproduct)(Data$dEither.Left)))(traverse4(f));
          const $4 = Control$dSemigroupoid.composeImpl($2.map(Control$dSemigroupoid.composeImpl(Data$dFunctor$dCoproduct.Coproduct)(Data$dEither.Right)))(traverse5(f));
          return v2 => {
            if (v2.tag === "Left") { return $3(v2._1); }
            if (v2.tag === "Right") { return $4(v2._1); }
            $runtime.fail();
          };
        };
      },
      sequence: dictApplicative => {
        const $2 = dictApplicative.Apply0().Functor0();
        const $3 = Control$dSemigroupoid.composeImpl($2.map(Control$dSemigroupoid.composeImpl(Data$dFunctor$dCoproduct.Coproduct)(Data$dEither.Left)))(dictTraversable.sequence(dictApplicative));
        const $4 = Control$dSemigroupoid.composeImpl($2.map(Control$dSemigroupoid.composeImpl(Data$dFunctor$dCoproduct.Coproduct)(Data$dEither.Right)))(dictTraversable1.sequence(dictApplicative));
        return v2 => {
          if (v2.tag === "Left") { return $3(v2._1); }
          if (v2.tag === "Right") { return $4(v2._1); }
          $runtime.fail();
