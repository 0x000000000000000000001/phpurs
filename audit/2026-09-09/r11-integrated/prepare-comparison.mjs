...
await build({
  ...
  plugins: [{ name: 'disable-thunk-fusion', setup(b) {
    b.onLoad({ filter: /Phpurs\.ThunkFusion\/index\.js$/ }, () => ({ contents: "import {empty} from '../Data.Set/index.js'; export const optimize = module_ => ({module_, privateNames: empty});", loader: 'js', resolveDir: path.join(backend, 'output/Phpurs.ThunkFusion') }));
  }}],
});
