...
await build({
  ...
  plugins: [{ name: 'disable-nullable-constructors', setup(b) {
    b.onLoad({ filter: /Phpurs\.NullableConstructors\/index\.js$/ }, () => ({ contents: "import {Nothing} from '../Data.Maybe/index.js'; export const layout = _ => Nothing.value; export const lower = _ => file => file;", loader: 'js', resolveDir: path.join(backend, 'output/Phpurs.NullableConstructors') }));
  }}],
});
...
const active = spawnSync(path.join(backend, 'bin/phpurs'), ['--output', path.join(integrated, 'output'), '--main', 'App'], { cwd: worktree, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024 });
