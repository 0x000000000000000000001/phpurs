  ...
  if (0 < indexedAndSorted.length) {
    return Data$dFunctor.arrayMap(Data$dTuple.snd)(sortWith(Data$dOrd.ordInt)(Data$dTuple.fst)((() => {
      ...
      for (const v1 of indexedAndSorted) {
        const $0 = comp(Control$dSemigroupoid.composeImpl(Data$dTuple.snd)(Control$dSemigroupoid.composeImpl(fromJust)(last))(result))(v1._2);
...
const filterA = dictApplicative => {
  ...
  return p => Control$dSemigroupoid.composeImpl($0.map(mapMaybe(v => {
    if (v._2) { return Data$dMaybe.$Maybe("Just", v._1); }
    return Data$dMaybe.Nothing;
  })))(traverse1(x => $0.map(Data$dTuple.Tuple(x))(p(x))));
};
...
const nubByEq = eq2 => xs => {
  const arr = [];
  for (const x of xs) {
    if (Control$dSemigroupoid.composeImpl(Data$dHeytingAlgebra.boolNot)(any(v => eq2(v)(x)))(arr)) { arr.push(x); }
  }
  return arr;
};
