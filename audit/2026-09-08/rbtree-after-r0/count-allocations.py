...
rows = {}
for name in ['baseline', 'inline-balance']:
    ...
    result = subprocess.run(['php', '-d', 'opcache.enable_cli=0', str(root / 'runner.php'), str(output), 'single'], text=True, capture_output=True)
...
assert all(rows['baseline'][k] == rows['inline-balance'][k] for k in ['R','B','E','T','ins']), rows
assert rows['baseline']['balance'] == 2183976 and rows['inline-balance'].get('balance',0) == 0, rows
