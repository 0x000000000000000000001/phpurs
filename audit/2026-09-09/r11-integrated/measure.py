"""Fresh sequential processes, reversed order, original Bench.purs clock."""
import json
...
prior = root.parent.parent/'2026-09-08'
runner = prior/('lazy/runner.php' if mode == 'bench' else 'r10-integrated/rbtree-runner.php')
...
rows = []
for number, order in enumerate([['baseline','integrated'], ['integrated','baseline']], 1):
    for name in order:
        command = flags+[str(runner), str(root/'raw'/name/'output'), mode]
        ...
        if mode == 'full':
            ...
            row.update(outputs=outputs, times_us=times, lazy_ms=times[11]/1000,
                       rbtree_ms=times[8]/1000, total_ms=sum(times)/1000)
        else:
            assert row['result'] == '1000000'
            row['lazy_ms'] = row['best_of_ten_us']/1000
        rows.append(row)
        (root/f'{mode}-results.json').write_text(json.dumps(rows, indent=2)+'\n')
        print(json.dumps({k:row[k] for k in ['variant','round','lazy_ms','total_ms'] if k in row}), flush=True)
