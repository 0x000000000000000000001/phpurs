...
for (const module of await fs.readdir(path.join(control, 'output'))) {
  const filename = path.join(control, 'output', module, 'corefn.json');
  try {
...
await build({
  ...
  plugins: [{ name: 'disable-tail-inline', setup(b) {
    b.onLoad({ filter: /Phpurs\.TailInline\/index\.js$/ }, () => ({ contents: 'export const optimize = file => file;', loader: 'js' }));
  }}],
});
...
console.log('Control regenerated with TailInline disabled; integrated output copied unchanged.');
