...
names = ['PartialBindings', 'CopyCleanup', 'NullableRegion', 'ImmediateThunkFusion', 'InternalEnumRegion', 'CompactClosureLoops', 'FunctionFFIBoundary', 'TerminalInlining', 'UnaryCallableCaptures', 'BranchBindingSlots', 'RecursiveDictionaryInitialization', 'DeepCase', 'ShadowedRename', 'ShadowedTCOLet', 'TCOCase', 'TCOMutRec', 'CaseMultipleExpressions']
results = []
for name in names:
    ...
    with (audit / f'fixture-{name}.log').open('w') as log:
        ...
        if name == 'PartialBindings':
            assert '__phpurs_bind_0_assemble' in php, 'The fixture must exercise the binding pass'
            assert "$GLOBALS['Main___phpurs_bind_" not in php, 'No private globals'
