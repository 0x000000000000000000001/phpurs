    ...
    in case isTailCallTo of
      Just index ->
        let
          ...
          popStmts = map (\\ctx -> PhpAssign ctx.doneVar (PhpBoolean true)) (Array.take (index + 1) loopCtx)
...
abs_new = """  Abs args body ->
    let
      ...
      resBody = translateExprImpl recVars namedBound bound Nothing Nothing true nextId body
    in
...
# Replace all "Nothing" passed to loopCtx with "[]" in remaining translateExprImpl calls
import re
