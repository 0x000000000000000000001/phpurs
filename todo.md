...
3. Profil LazyEvaluation terminé : le prototype vers 50 ms change le contrat FFI et reste retiré. R8 est intégré pour les boucles dont le trajet fonctionnel interne est prouvé : Lazy ~50 ms, total régénéré 328–332 ms ; 1 000 conversions pour un million de nœuds. Les cas avec fuite FFI/Foreign/record restent sur les Closure existantes. Reclasser les coûts avant tout élargissement : RBTree reste dominant. RBTree reste le premier coût ; les enveloppes d'appels seules et le GC ne donnent pas un gros gain dans les expériences actuelles.
...
**Première optimisation générique intégrée et mesurée.** Le [bilan R8 intégré](audit/2026-09-08/r8-integrated/report.md) donne LazyEvaluation **49,540–49,886 ms** contre **104,907–105,704 ms** dans les contrôles complets ; total **327,679–331,761 ms** contre **372,677–399,379 ms**, avec variation de RBTree identique par empreinte. bin/php/run -c passe et le PHP actif correspond à la variante mesurée.

Profil initial : LazyEvaluation construit puis force un million de closures capturantes. Remplacer les captures par des champs d'un objet invocable descend vers 50 ms, contre environ 90–102 ms pour les closures, sans modifier la chaîne ni le million d'appels. Le contrôle d'arité seul et le collecteur de cycles ne sont pas les coûts dominants. Voir le [rapport](audit/2026-09-08/lazy/report.md).

Le prototype générique (fonctions locales à un argument, captures par valeur) passe les 14 benchmarks mais échoue au contrat FFI : Foreign.typeOf/tagOf retourne object/Object au lieu de function/Function. Aff contient également un test instanceof Closure. Le prototype reste uniquement dans le dossier d'audit ; Printer.purs et le PHP courant ont été restaurés. Emballer chaque objet dans une vraie Closure réduit fortement le gain. Le [nouvel essai de conversion de racine](audit/2026-09-08/r8-root/report.md) conserve le gain avec seulement 1 000 conversions : Lazy isolé 46,028–46,349 ms contre 87,350–88,710 ms ; total des 14 tests 321,164–324,011 ms contre 352,631–363,984 ms. Ce relevé précédent concerne la copie diagnostique ; les mesures de l'intégration figurent en tête de R8.
...
- [x] Tester sur copie isolée les captures compactes et la conversion de racine : 1 000 conversions pour 1 000 000 nœuds/invocations, résultat inchangé. Une variante ne convertit que les racines effectivement retournées : zéro conversion dans la surapplication du benchmark, 1 000 sur les racines demandées séparément ; pas d'avantage temporel démontré sur la variante simple.
- [x] Vérifier profondeurs 0/1/2/17/1000, identité à zéro, racines réutilisées, captures mutables, exceptions, Closure/Foreign, records et curry/surapplication. Les quatorze sorties passent dans les deux ordres. Les fichiers PHP actifs restent identiques ; mesurer le total observé (321–326 ms sur les variantes), sans corriger artificiellement la variation de RBTree.
- [x] Intégrer CompactLoops : signature TAST monomorphe unaire à argument/résultat scalaire, un accumulateur fonctionnel compatible, alias/captures suivis dans la boucle TCO, conversion de la racine au retour public. Budget 4 096 nœuds au niveau du corps et 64 passages de fermeture ; conserver l'original si la preuve échoue. Conserver les Closure actuelles lorsque les usages FFI, Foreign, records/collections ou exports ne sont pas couverts ; garder le contre-exemple des callbacks imbriqués.
- [x] Ajouter CompactClosureLoops, avec une boucle éligible capturant chaque numéro de tour et une boucle refusée car elle expose les intermédiaires à la FFI. Les 12 fixtures passent en PHP et JS ; 4 scripts directs passent, dont 21 scénarios de refus pour la nouvelle passe. Régénération complète et 14 sorties validées ; même million de nœuds/invocations, 1 000 conversions. Seul Test.LazyEvaluation change dans cette comparaison, sans nom Test.* dans le critère.
- [x] Reprofiler les coûts après R8/R9 : RBTree reste dominant ; les objets auxiliaires et tests de constructeurs donnent la piste R10. Voir le [nouveau bilan](audit/2026-09-08/rbtree-after-r9/report.md). Toute extension de R8 aux captures/résultats/agrégats encore exclus reste une preuve et une mesure distinctes.
...
Cette première étape R9 est terminée dans le checkout, sans commit de cette étape. L'extension aux cibles typées/non feuilles ou aux appels non terminaux reste hors du périmètre validé. Le TAST v3 conserve types, instanciations et layouts ; cette passe agit sur le flot de contrôle du PHP AST sans nouvelle représentation FFI. Réduire fortement les allocations d'arbres persistants demanderait une preuve distincte d'absence d'alias. R8 est désormais intégré pour le trajet fonctionnel interne prouvé ; le total régénéré est de 328–332 ms dans ses contrôles.

## R10 — P1 — Représentation scalaire des ADT internes

**Extension nullable intégrée.** La preuve de région fermée accepte aussi les layouts monomorphes à un constructeur vide et un constructeur avec champs, même sans enum. Les noms privés prouvés guident `NullableConstructors` dans l'AST PHP : allocation vide vers null, tests vers `=== null`/`!== null`, constructeur avec champs toujours objet. Un dépassement de budget conserve la représentation objet pour tout le module. Après `bin/php/run -c`, les paires donnent total 165,547–167,945 ms contre 196,398–198,500 ms, RBTree 146,033–148,072 ms contre 176,845–179,300 ms et pic mémoire 26 contre 34 MiB. Voir le [bilan intégré](audit/2026-09-09/r10-null-integrated/report.md).

**Première passe intégrée.** Après régénération complète, RBTree passe de **249–253 à 176–178 ms** et le total de **316,7–320,2 à 241,6–242,7 ms** : **23,35–24,54 % de temps total en moins**. Les couleurs sont scalaires uniquement dans les régions fermées prouvées par le TAST ; les propriétés des classes internes évitent les contrôles PHP redondants. Les représentations et contrôles publics sont préservés. `bin/php/run -c`, les 13 fixtures PHP/JS, les suites AST R0/R8/R9 et les contrôles d'arbres passent. Voir le [bilan d'intégration](audit/2026-09-08/r10-integrated/report.md).

Repérage antérieur sur copies : après R9, couleurs `R/B` scalaires seules à 189–190 ms contre 249–251 ms en RBTree isolé. L'expérience globale ajoutant `E = null` donnait 223–227 ms au total contre 307–308 ms, mais modifiait des observations publiques. Les valeurs et anciennes racines étaient vérifiées sur 1 285 insertions ; même nombre de nœuds `T` et d'appels récursifs. Voir le [bilan initial](audit/2026-09-08/rbtree-after-r9/report.md).

Le README au moment de l'intégration R10 conservait le relevé utilisateur **316,45 ms**, avec RBTree **247,881 ms** ; ses témoins étaient proches. Après R11, le README principal affiche **188,94 ms / RBTree 169,936 ms** et celui du worktree **196,11 ms / 176,562 ms**. Le natif optimisé historique donne RBTree 123,096 ms et total 200,45 ms ; le natif de style fonctionnel ne réalise pas exactement la même charge RBTree. Les expériences sur copies et les mesures du générateur intégré sont distinguées dans les bilans.

Le TAST décrit explicitement `Color = R | B` et `Tree = E | T Color Tree Int Tree`, sans recherche heuristique des arités. La substitution globale casse cependant des observations PHP : classes des champs, arbre vide et sérialisation. Les singletons ont un gain moindre et changent aussi l'identité des objets.

**Périmètre interne et prototype préalables** : [bilan des frontières R10](audit/2026-09-08/r10-boundary/report.md). La fixture passait en PHP/JS, 14 scénarios de refus étaient vérifiés et les copies internes préservaient les fonctions, classes et sérialisations publiques testées. Sur RBTree, les seules couleurs scalaires internes donnaient **189–197 ms**, total **253–261 ms** contre **304–310 ms**, soit 15,8–16,6 % de temps total en moins. Les 301 modules PHP actifs étaient alors inchangés.

- [x] Profiler le JIT après R9, puis mesurer séparément singletons, couleurs scalaires, arbre vide nullable et tests de nullité ; deux ordres, sans cache OPcache partagé.
- [x] Vérifier les 14 sorties, les formes canoniques des arbres, la conservation des anciennes racines et les comptes réels de constructions ; constater les différences visibles à la frontière PHP.
- [x] Ajouter InternalEnumRegion : groupe monomorphe à entrée/sortie scalaire, FFI directe et imbriquée, callback conservé, appel inconnu et exports. L'analyse TAST prototype refuse les sorties non prouvées, les types/formes inconnus et les dépassements de budget ; aucune sélection par nom de benchmark.
- [x] Expérimenter les copies internes pour les seules couleurs en conservant les fonctions et valeurs publiques. Vérifier appels partiels, anciennes racines, sérialisation PHP, FFI et compteurs des chemins réellement exécutés.
- [x] Adapter le périmètre au vrai RBTree : identifier dans act la sous-expression scalaire `show (depth (buildTree dummy E))`, avec dummy :: Int comme seule variable libre. Conserver l'enveloppe Effect ; copies internes sur PHP figé, 14 résultats, 1 285 arbres internes/publics et comptes d'allocations vérifiés. Les 253–261 ms sont ceux de ce prototype.
- [x] Intégrer EnumRegions sur le TAST optimisé : analyse et duplication structurelles, noms frais, appels saturés, région à résultat/entrées scalaires, budgets et repli conservateur. Garder les fonctions et classes publiques ; les workers internes n'ont pas de binding `$GLOBALS`. Rejouer les contre-exemples, les contrôles de propriétés publiques et R0/R8/R9 ; régénérer avec bin/php/run -c et comparer à passe désactivée. Résultat : 241,6–242,7 ms contre 316,7–320,2 ms, pic mémoire 34 contre 42 MiB, +56 290 octets pour RBTree. Les 499 934 allocations de couleurs disparaissent ; les 2 583 932 nœuds T et 200 001 E restent alloués.
- [x] **Suite R10, prototype nullable après R11.** Layout TAST monomorphe `Tree = E | T Color Tree Int Tree` vérifié dans la région privée existante. Sur copies : `E = null` seul donne RBTree 159–160 ms contre 176–178 ms en isolé ; ajouter les tests `T !== null` donne 140–144 ms. Dans la suite complète : RBTree 138–140 ms, total 157–159 ms contre 190–193 ms, pic mémoire 26 contre 34 MiB. Les 200 001 allocations de E disparaissent ; 2 583 932 T et 2 283 976 appels à ins restent. Les 14 résultats, 1 285 insertions, 1 290 paires d'anciennes racines, invariants rouge/noir et sérialisations publiques passent. Voir le [bilan](audit/2026-09-09/r10-null/report.md). Aucun changement du générateur ni des 301 modules actifs.
- [x] **Intégrer la représentation nullable privée.** `EnumRegions` vérifie layouts et usages dans le TAST ; `NullableConstructors` abaisse leurs seuls noms privés dans l'AST PHP. Constructeurs avec champs conservés, y compris pour deux domaines nullable imbriqués ; cas ambigus/inconnus et sorties publiques refusés, budgets atomiques par module. Sept suites AST, quinze fixtures PHP/JS, invariants rouge/noir, 1 285 insertions et 1 290 paires d'anciennes racines passent. `bin/php/run -c` réussit ; désactiver la nouvelle transformation retrouve les 302 fichiers PHP précédents. Seul RBTree change, et son préfixe public reste identique. Les 200 001 E disparaissent ; les nœuds T et appels récursifs restent en même nombre. Total mesuré : 15,39–15,71 % de moins, chiffres distincts du prototype.
...
- [x] Vérifier les 14 résultats, 30 cas numériques, l'ordre des arguments, les effets ordinaires, l'identité des exceptions, les appels publics partiels/répétés et les FFI Closure/Foreign. Conserver deux contre-exemples : inspection de pile et raccourci arithmétique après débordement PHP.
- [x] Lire le TAST du site : profondeur littérale positive, fonction initiale connue `Unit -> Int`, résultat `Int`. Refuser dans le diagnostic six mutants de frontière/type. Ce contrôle ciblé n'est pas encore une preuve générique des corps récursifs.
