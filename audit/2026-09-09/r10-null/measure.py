...
names = sys.argv[2:] or ['baseline', 'nullable-leaves', 'null-checks']
...
for number, order in enumerate([names, list(reversed(names))], 1):
    for name in order:
        ...
        print(json.dumps({k:row[k] for k in ['variant','round','rbtree_ms','total_ms','peak_memory_bytes'] if k in row}), flush=True)
